...
const TripsPage = () => {
  ...
  const tripsContent = loading ? (
    ...
    filteredBookings.map((booking) => <TripCard key={booking.bookingId} booking={booking} />)
  );
        ...
        <div className="grid gap-6 lg:grid-cols-[1.5fr_1fr]">
          <section>
            ...
            <p className="mt-1 text-muted-foreground">
              Reservations created in this browser are stored locally and refreshed from the booking service.
            </p>

            <div className="mt-6 flex gap-2 overflow-x-auto">{tabButtons}</div>

            <div className="mt-6 space-y-4">{tripsContent}</div>
          </section>
          ...
          <aside className="rounded-2xl border border-border bg-card p-6 shadow-sm">
            ...
            <p className="mb-4 text-sm text-muted-foreground">
              Preserves the legacy booking-status lookup flow even after removing the old static guest page.
            </p>
            <div className="space-y-4">
              ...
              {lookupError && (
                <div className="rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700">{lookupError}</div>
              )}
              {lookupBooking && (
                <DetailTable
                  fields={[
